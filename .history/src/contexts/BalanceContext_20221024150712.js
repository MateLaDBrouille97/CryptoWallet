...
const BalanceContextProvider = ({ children }) => {
    ...
    const getBalance = async () => {
        if (connector.connected) {
            ...
            try {
                await web3.eth.getBalance(connector.accounts[0]).then(balance => { setBal(balance) }).then(result => await web3.utils.fromWei(result,"ether")).then(eth => { setEth(eth) });
                const ethB = await web3.utils.fromWei(bal, "ether");
                setEth(ethB);
